...
#'
#' @param testData Gene expression data for test group.
#' @param normalData Gene expression data for normal group.
#' @param gene_symbol Gene symbol list.
#' @param upFc_thres Up-regulated log-fold-change threshold.
#' @param dnFc_thres Down-regulated log-fold-change threshold.
...
GOAnalytics <-
  function(testData,
    ...
    idx_up <-
      which(
        GO_up[, 3] == "BP" &
          as.numeric(GO_up[, 4]) <= 1000 &
          as.numeric(GO_up[, 4]) >= 10 & GO_up[, 5] <= pv_thres
      )
    idx_dn <-
      which(
        GO_dn[, 3] == "BP" &
          as.numeric(GO_dn[, 4]) <= 1000 &
          as.numeric(GO_dn[, 4]) >= 10 & GO_dn[, 5] <= pv_thres
      )
...
#' Go ontology util function
#'
#' @param gs1 Input gene set list.
#' @param all_gene Gene symbol for all genes.
#'
#' @return
#' @export
#'
...
#' Generate gene ontology network.
#'
#' @param gs0 Gene set list.
#' @param GO1 GO results.
#'
#' @return
#' @export
#'
