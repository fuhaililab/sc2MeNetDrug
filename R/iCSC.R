#' Compute DEG information using Seurat package with two different tests.
#'
#' @param data Seurat object.
#' @param ident1 test groups.
#' @param ident2 normal/control groups.
#'
#' @return
#' @export
#'
...
seurat_DEG <-
  function(data,
    ...
    intersect_gene <-
      intersect(rownames(result1),
                rownames(result2))
    result <-
      data.frame(
        logFC = result1[intersect_gene, 2],
        pct1 = result1[intersect_gene, 3],
        pct2 = result1[intersect_gene, 4],
        wlicox_p_value = result1[intersect_gene, 1],
        wlicox_p_adjust = result1[intersect_gene, 5],
        bimod_p_value = result2[intersect_gene, 1],
        bimod_p_adjust = result2[intersect_gene, 5]
      )
    rownames(result) <- intersect_gene
    result
  }


#' inter-cell communication discovery function
#'
#' @param data scRNA gene expression matrix.
#' @param gene_symbol Gene symbol list.
#' @param group_list Cell group list.
#' @param type_list Cell type list .
#' @param normalGroup List of group used as normal/control samples.
#' @param testGroup List of group used as test samples.
#' @param fc_thres Log-fold-change threshold.
#' @param pv_thres P-value threshold.
#' @param cell_type1 The first cell type for inter-cell communication network.
#' @param cell_type2 The second cell type for inter-cell communication network.
#' @param outputDir Directory for saving all results.
#' @param ligRecDatabase Ligand-receptor network database.
#' @param TfTargetInteraction Transcriptional factor-targets network databse.
#' @param keggInfo KEGG databse.
#' @param STRING STRING database.
#' @param resource Which signaling network to use (KEGG/STRING) in the computation.
#' @param useOld If true, load saved results.
#' @param padjust If true, use adjusted P-value instead of original P-value.
  ...
  function(data,
    ...
    if (!useOld) {
      if (!is.null(group_list)) {
        ...
        normal_group_type1 <-
          paste(normalGroup, rep(cell_type1, num_normal_group), sep = "_")
        normal_group_type2 <-
          paste(normalGroup, rep(cell_type2, num_normal_group), sep = "_")
        test_group_type1 <-
          paste(testGroup, rep(cell_type1, num_test_group), sep = "_")
        test_group_type2 <-
          paste(testGroup, rep(cell_type2, num_test_group), sep = "_")
        each_condition <- unique(Idents(data))
        normal_group_type1 <-
          intersect(normal_group_type1, each_condition)
        normal_group_type2 <-
          intersect(normal_group_type2, each_condition)
        test_group_type1 <-
          intersect(test_group_type1, each_condition)
        test_group_type2 <-
          intersect(test_group_type2, each_condition)
        if (length(normal_group_type1) == 0 ||
            length(normal_group_type2) == 0 ||
            length(test_group_type1) == 0 ||
            length(test_group_type2) == 0) {
      ...
      cell_type1_result <-
        cell_type1_result[order(cell_type1_result$logFC, decreasing = T),]
      cell_type2_result <-
        cell_type2_result[order(cell_type2_result$logFC, decreasing = T),]
      
      
      genesInformation <-
        list(cell_type1_result,
             cell_type2_result,
             cell_type1,
             cell_type2)
      save(genesInformation,
           file = paste0(netDir, "/genesInformation.RData"))
    } else{
    ...
    cell_type1_ligands <-
      cell_type1_result[rownames(cell_type1_result) %in% Ligand,]
    cell_type1_receptors <-
      cell_type1_result[rownames(cell_type1_result) %in% Receptor,]
    
    cell_type2_ligands <-
      cell_type2_result[rownames(cell_type2_result) %in% Ligand,]
    cell_type2_receptors <-
      cell_type2_result[rownames(cell_type2_result) %in% Receptor,]
    ...
    save(ligRecInformation,
         file = paste0(netDir, "/ligRecInformation.RData"))
    ...
    if (resource == "KEGG") {
      gen_function <- network_kegg
      signaling_database <- keggInfo
    }
    else {
      gen_function <- network_string
      signaling_database <- STRING
...
#' Generate inter-cell communication network using KEGG database.
#'
#' @param gSyms Gene symbol list.
#' @param type1_genes Selected gene list from cell type 1.
#' @param type2_genes Selected gene list from cell type 2.
#' @param fcCellType1 Log-fold-change for all selected genes of cell type 1.
#' @param pvCellType1 P-value for all selected genes of cell type 1.
#' @param fcCellType2 Log-fold-change for all selected genes of cell type 2.
#' @param pvCellType2 P-value for all selected genes of cell type 2.
#' @param fc_thres Log-fold-change threshold.
#' @param pv_thres P-value threshold.
#' @param ligRecDatabase Ligand-receptor database.
#' @param TfTargetInteraction Transcriptional factor-target interaction database.
#' @param keggInfo KEGG database.
#'
#' @return
#' @export
#'
...
network_kegg <-
  function(gSyms,
    ...
    # Set Parameters
    thresStromaLigUpFc <-
      fc_thres  # Threshold to identify activated Lig from Stroma Cells (fold change and p-value)
    thresStromaLigUpPv <- pv_thres
    
    thresStromaLigExpressedFc <-
      0  # Threshold to identify expressed Lig from Stroma Cells (fold change and p-value)
    thresStromaLigExpressedPv <- 1.0
    
    thresEpiRecUpFc <-
      fc_thres  # Threshold to identify activated Receptor from Epi Cells (fold change and p-value)
    thresEpiRecUpPv <-
      pv_thres  # for identify the up-regulated (actived) receptors
    
    thresEpiGeneUpFc <-
      fc_thres# Threshold to identify Up-regulated genes from Epi Cells (fold change and p-value)
    thresEpiGeneUpPv <-
      pv_thres	 # for the fisher's test p-value of pathy encrichment
    
    thresEpiRecExpressedFc <-
      0  # Threshold to identify expressed Receptor from Epi Cells (fold change and p-value)
    thresEpiRecExpressedPv <-
      1.0  # for identify the expressed receptors
    
    thresEpiGeneExpressedFc <-
      0  # Threshold to identify expressed gene from Epi Cells (fold change and p-value)
    thresEpiGeneExpressedPv <-
      1.0  # for identify the expressed genes
    ...
    idx1a <-
      which(
        fcLig1 >= thresStromaLigUpFc &
          pvLig1[, 1] <= thresStromaLigUpPv &
          pvLig1[, 2] <= thresStromaLigUpPv
      )
    ligUp1 <- gLig1[idx1a]
    ...
    idx1a <-
      which(fcRec1 >= thresEpiRecUpFc &
              pvRec1[, 1] <= thresEpiRecUpPv &
              pvRec1[, 2] <= thresEpiRecUpPv)
    recUp1 <- gRec1[idx1a]  # Up-Expressed Receptor in Epi
    
    if (length(unique(ligUp1)) == 0 &&
        length(unique(recUp1)) == 0) {
    ...
    upLig_expRec <-
      ligRecDatabase[(vLigand %in% ligUp1u) &
                       (vReceptor %in% recExp1u), ]
    expLig_upRec <-
      ligRecDatabase[(vLigand %in% ligExp1u) &
                       (vReceptor %in% recUp1u),]
    lig_rec_network <- unique(rbind(upLig_expRec, expLig_upRec))
    #downstram network
    #cell type 2 network information
    netInfo <-
      generateKeggNetInfoV1(type2_genes, fcCellType2, keggInfo)
    #activate signaling network among cell type2
    activated_network <-
      generateKeggActivationNetworkV1(netInfo, fc_thres)
    #downstream network for cell type2
    if (dim(lig_rec_network)[1] != 0) {
      downstream_network <-
        generateKeggReceptorNetworkV1(netInfo, unique(as.character(lig_rec_network[, 2])), fc_thres)
    } else{
      downstream_network <- matrix(ncol = 3)
      colnames(downstream_network) <-
        c('source', 'target', 'PathwayName')
    }
    ...
    if (!is.null(activated_network)) {
      activated_network[, 1] <-
        paste(activated_network[, 1], activated_network[, 3], sep = "_")
      activated_network[, 2] <-
        paste(activated_network[, 2], activated_network[, 3], sep = "_")
      activated_edges <- data.frame(activated_network)
      nodes <-
        unique(c(
          as.character(activated_edges[, 1]),
          as.character(activated_edges[, 2])
        ))
      nodeSymbol <-
        as.character(sapply(nodes, function(x) {
      ...
      activated_nodes <-
        activated_nodes[order(as.character(activated_nodes$parent)),]
    ...
    if (!is.null(downstream_network)) {
      ...
      downstream_gene <-
        unique(c(downstream_network[, 1], downstream_network[, 2]))
      lig_rec_network <-
        lig_rec_network[lig_rec_network[, 2] %in% downstream_gene,]
      lig_rec_network <-
        rbind(c("source", "target"), lig_rec_network)
      
      ligList <- unique(as.character(lig_rec_network[, 1]))
      recList <- unique(as.character(lig_rec_network[, 2]))
      lig_rec_network[, 1] <-
        paste(lig_rec_network[, 1], "Ligand", sep = "_")
      lig_rec_network[, 2] <-
        paste(lig_rec_network[, 2], "Receptor", sep = "_")
      
      recIdx1 <- which(downstream_network[, 1] %in% recList)
      downstream_network[recIdx1, 1] <-
        paste(downstream_network[recIdx1, 1], "Receptor", sep = "_")
      recIdx2 <- which(downstream_network[, 2] %in% recList)
      downstream_network[recIdx2, 2] <-
        paste(downstream_network[recIdx2, 2], "Receptor", sep = "_")
      
      tfIdx1 <- which(downstream_network[, 1] %in% TFs)
      downstream_network[tfIdx1, 1] <-
        paste(downstream_network[tfIdx1, 1], "TranscriptionFactor", sep = "_")
      tfIdx2 <- which(downstream_network[, 2] %in% TFs)
      downstream_network[tfIdx2, 2] <-
        paste(downstream_network[tfIdx2, 2], "TranscriptionFactor", sep = "_")
      
      targetIdx1 <- which(downstream_network[, 1] %in% Targets)
      downstream_network[targetIdx1, 1] <-
        paste(downstream_network[targetIdx1, 1], "Target", sep = "_")
      targetIdx2 <- which(downstream_network[, 2] %in% Targets)
      downstream_network[targetIdx2, 2] <-
        paste(downstream_network[targetIdx2, 2], "Target", sep = "_")
      
      num_edge <- dim(downstream_network)[1]
      linkNodeIdx1 <-
        which(!c(1:num_edge) %in% c(recIdx1, tfIdx1, targetIdx1))
      downstream_network[linkNodeIdx1, 1] <-
        paste(downstream_network[linkNodeIdx1, 1], "LinkNode", sep = "_")
      linkNodeIdx2 <-
        which(!c(1:num_edge) %in% c(recIdx2, tfIdx2, targetIdx2))
      downstream_network[linkNodeIdx2, 2] <-
        paste(downstream_network[linkNodeIdx2, 2], "LinkNode", sep = "_")
      
      
      communication_edges <-
        data.frame(rbind(lig_rec_network, downstream_network))
      communication_edges <- communication_edges[-1,]
      
      nodes <-
        unique(c(
          as.character(communication_edges[, 1]),
          as.character(communication_edges[, 2])
        ))
      nodeSymbol <-
        as.character(sapply(nodes, function(x) {
          strsplit(x, split = "_")[[1]][1]
      ...
      communication_nodes <-
        communication_nodes[order(as.character(communication_nodes$parent)),]
...
#' Generate inter-cell communication network using STRING database.
#'
#' @param gSyms Gene symbol list.
#' @param type1_genes Selected gene list from cell type 1.
#' @param type2_genes Selected gene list from cell type 2.
...
network_string <-
  function(gene_symbol,
    ...
    G_String <-
      generateSigNetwork_setNetWeight(STRING, type2_genes, fcCellType2)
    gs_1a <- intersect(node_String, receptor_list)
    gs_select1 <-
      type2_genes[fcCellType2 > fc_thres &
                    pvCellType2[, 1] < pv_thres &
    ...
    if (length(downstream_network) < 3) {
      ...
      downstream_network <-
        downstream_network[-idx_t,] # remove the 'Source'/'Target' row
...
generateKeggReceptorNetworkV1 <- function(netInfo, Rs, T0) {
  ...
  if (1 == 1) {
    ...
    for (j in 1:nt) {
      ...
      #
      if (zt1a >= T0 & zt1b > v_t_a[lt]) {
        # activation
        if (length(which(zt2[, 1] %in% Rs |
                         zt2[, 2] %in% Rs)) > 0) {
          # include the receptors
          path_t <- rbind(path_t, zt2)
        }
      }
      if (zt1a >= T1 & zt1b < v_t_ai[lt]) {
        # inhibition
...
#'
#' @param gSyms Gene symbol list.
#' @param fc_tmp Log-fold-change list for genes.
#' @param keggInfo KEGG database.
#'
#' @return
#' @export
...
generateKeggNetInfoV1 <- function(gSyms, fc_tmp, keggInfo) {
  ...
  if (1 == 1) {
    ...
    sigPath <- c()
    sigPath <-
      getSubPathwayKeggV2a1(fc_tmp, gSyms, KeggPathways, KeggGraphs, lPath)
    pt1 <- sigPath$pathTmp
    lPath <- sigPath$lPath
    ...
    #
    v_t_a <-
      rep(0.25, 50)  # activation threshold for the signaling paths with different lengths
    v_t_ai <- rep(-0.25, 50) # inhibition score threshold
    #
    s_p_a <-
      matrix(0, 50, 3) # to calculate the Real # of signaling pathways, mean-activation, mean-inhibition
    ...
    for (i in 1:50) {
      ...
      s_p_a[i,] <-
        c(length(vt1), mean(vt1[vt1 > 0]), mean(vt1[vt1 < 0]))
      # vertex score
      vt1 <- path_score_v[[i]]
      s_p_v[i,] <- c(length(vt1), mean(vt1[vt1 > 0]))
    }
    v_t_a[c(11:50)] <-
      5.0  # will not consider the paths with more than 11 nodes
    v_t_ai[c(11:50)] <- -5.0
    #
    netInfo <- list()
    netInfo[[1]] <- pt1
...
#' Generate sub pathway with KEGG
#'
#' @param fc1 Log-fold-change list.
#' @param gSyms Gene symbol list.
#' @param KeggPathways KEGG pathway information.
#' @param KeggGraphs KEGG network information.
#' @param lPath interaction path.
#'
#' @return
#' @export
#'
...
getSubPathwayKeggV2a1 <-
  function(fc1,
    ...
    for (j1 in 1:n_pathway) {
      # for all signaling pathways
      ...
      #
      if (min(n_rec1, n_tf1) < 1) {
        # no possible Receptor - TF path
        next
      }
      # print(j1)
      for (j2 in 1:n_rec1) {
        ...
        paths <-
          get.shortest.paths(gTmp, pf1, tf_set1, mode = 'out')
        ...
        if (nPath > 0) {
          ...
          for (k in 1:nPath) {
            ...
            st <-
              getScorePathwayKeggV1b(eTmp1, KeggPathways[[j1]], zScore, 0.0)
            xt <- list()
            xt[[1]] <- st
            xt[[2]]	<- eTmp1
            n_pt <- (n_pt + 1)
...
#' Compute score for the pathway.
#'
#' @param edgePath Edge path, nx2 dimension; each row is an edge
#' @param eType Edge type, e.g., activation, inhibition,
#' @param zScore Gene expression of genes, e.g., fold change, z-score, protein expression
#' @param alpha1 Weight of the contribution of the parent node
#'
#' @return
#' @export
#'
...
getScorePathwayKeggV1b <-
  function(edgePath, eType, zScore, alpha1) {
    ...
    for (i in 1:nEdge) {
      vt <-
        0.5 * abs((eScore[i, 1] + eType1[i + 1] * eScore[i, 2]))  # abs
      vScore <- vScore + vt
      #
    ...
    vScore <-
      vScore + 0.5 * eType1[i + 1] * eScore[i, 2] + 0.5 * eScore[1, 1]  # the last node and first node score
    vScore <- vScore / (nEdge + 1)
    #
    aScore <-
      aScore + 0.5 * eScore[i, 2] + 0.5 * eScore[1, 1]  # the last node and first node score
...
#'
#' @param G_t Network.
#' @param gs1a Gene list 1.
#' @param gs2a Gene list 2.
#'
#' @return
#' @export
#'
...
generateSigNetwork_v3a <- function(G_t, gs1a, gs2a) {
  ...
  gs1b <-
    intersect(gs1a, gene_g_t) # root genes; remove genes are not in the STRING databse
    ...
    for (i_gs in 1:1) {
      ...
      while (flag1 > 0) {
        ...
        if (length(V2) == 1) {
          ...
          paths_0 <-
            get.shortest.paths(ig_t0, V2[idx_t1b], gs_1[idx_t1a])
...
#'
#' @examples
generateSigNetwork_setNetWeight <-
  function(g_t, gene_symbol_0, fc_t0) {
    fc_t0 <- exp(fc_t0)
    library(igraph)
    #
    g_t <- cbind(as.character(g_t[, 1]), as.character(g_t[, 2]))
    g_t <-
      g_t[-which(g_t[, 1] == g_t[, 2]),]  # remove the self-connection
    ...
    for (i in 1:n_V0) {
      # update the edge; assign the fold change to the nodes
      vt <- fc_t0a[i]
