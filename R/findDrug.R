#drug bank drug mapping function
findDrug<-function(network_nodes,network_edges,drugBankInteraction,drugBankInformation){
  ...
  for (i in 1:length(nodes_name)){
    node<-nodes_name[i]
    node_drug_list<-drugBankInteraction[which(drugBankInteraction$target==node),]
    ...
    if(nrow(node_drug_list)!=0){
      ...
      drug_set_for_gene<-c(drug_set_for_gene,paste(node_drug_list$drug_name,collapse = ",  "))
      for (j in 1:num_drug){
        drug<-as.character(node_drug_list[j,1])
        drug_gene_list<-paste(drugBankInteraction[drugBankInteraction$drug_name==drug,2],collapse=", ")
  ...
  drug_table<-data.frame(drug_name=drug_list,drug_gene_set=drug_gene_set,stringsAsFactors = F)
  drug_table<-unique(drug_table)
  drug_table<-merge(drug_table,drugBankInformation,by.x="drug_name",by.y="drug_name",all.x=T)
  new_network_nodes<-cbind(network_nodes,drugSignal=drug_signal,drugList=drug_set_for_gene,numDrug=num_drug_list)
  ...
  drugClusteringResult<-drugClusteringChemicalStructure(drug_smiles_list)
  
  list(drug_table,final_data,drugClusteringResult[[1]],drugClusteringResult[[2]],drugClusteringResult[[3]])
}
...
drugClusteringChemicalStructure<-function(drug_list){
  ...
  drug_type<-sapply(drug_name_list,function(x){ifelse(x %in% unique_exemplars,"exemplar","normal")})
  drug_node<-data.frame(node=drug_index,type=drug_type,name=drug_name_list)
  return(list(drugCluster,drug_node,drug_network))
}



findDrug2<-function(network_nodes,gSymZs,rankMatrix,drug_mapping,topDrug,useFDAOnly,drugBank){
  ...
  es_score<- getPatient2DrugReverseDis2(geneSetUp, NULL, gSymZs, rankMatrix, n.probe.all, n.drg)
  es_score_rank <- es_score[order(es_score, decreasing=F)] # up.ks - down.ks; then negative relationship
  name_list<-names(es_score_rank)
  name_list<-sapply(name_list,function(x){substring(x,1,nchar(x)-3)})
  index<-c(1:length(es_score_rank))
  top<-data.frame(rankName=name_list,enrichment_score=as.vector(es_score_rank),drug_index=index)
  drug_table<-merge(top,drug_mapping,all.x=TRUE, by.x="rankName",by.y="rankName")
  drug_table<-drug_table[order(drug_table$drug_index),]
  drug_table<-drug_table[,c(1,2,5,10,11)]
  rownames(drug_table)<-index
  if(useFDAOnly==1){
    drug_table<-drug_table[toupper(drug_table$pert_iname)%in%toupper(FDAdrugs),]
  ...
  if(!is.null(drug_table)){
    ...
    drug_sim<-apply(sub_rankMatrix,2,cal_drug_es,gSymZs=gSymZs,rankMatrix=sub_rankMatrix)
    ...
    for (i in 1:length(drug_rank_names)){
      ...
      drug_names<-c(drug_names,paste0(as.character(infor$pert_iname),", ",as.character(round(infor[2],3))))
    }
    drug_type<-sapply(drug_rank_names,function(x){ifelse(x %in% unique_exemplars,"exemplar","normal")})
    drug_node<-data.frame(node=drug_index,type=drug_type,name=drug_names)
  }else{
    result<-NULL
    drug_node<-NULL
    drug_network<-NULL
...
cal_drug_es<-function(x,gSymZs,rankMatrix){
  ...
  n.probe.all <- dim(rankMatrix)[1]; 
  n.drg <- dim(rankMatrix)[2];
  getPatient2DrugReverseDis2(geneSet_up, geneSet_down, gSymZs, rankMatrix, n.probe.all, n.drg)
...
# -------------
getPatient2DrugReverseDis2 <- function(geneSetUp, geneSetDown, gSymZs, rankMatrix, n.probe.all, n.drg){ ###
  ...
  if (length(probe.set.down)>1 & sum(is.na(probe.set.down)==TRUE)<1){
    down.ks = apply(rankMatrix, 2, GSEA.EnrichmentScore2, gene.set=probe.set.down)
    down.ks = unlist(down.ks); # newly added1
  } else {
    down.ks = 0.0;
  }
