#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# continue working modules used for reload data from the working directory.
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
continueWork <- function(input, rv, session) {
  ...
  if (result1 == -1 && result2 == -1) {
    ...
    if (file.exists(paste0(rv$outputDir, "/lastUpRegIndexing.RData"))) {
      ...
      if (file.exists(paste0(groupDir, "/upRegNetwork.RData")) &&
          file.exists(paste0(groupDir, "/upStreamNetwork.RData"))) {
    ...
    if (file.exists(paste0(rv$outputDir, "/GO_result.RData"))) {
      ...
      GO_list <-
        c(as.character(rv$GO_up_table[, 1]),
          as.character(rv$GO_dn_table[, 1]))
      GO_name <-
        c(as.character(paste(
          rv$GO_up_table[, 1], rv$GO_up_table[, 2], sep = "  "
        )), as.character(paste(
          rv$GO_dn_table[, 1], rv$GO_dn_table[, 2], sep = "  "
        )))
    ...
    if (networkComputed == 1) {
      ...
      if (haveNetwork1 == 1) {
        ...
        display_json <-
          networkDataToJson(downstream_network_nodes1,
      ...
      if (haveNetwork2 == 1) {
        ...
        display_json <-
          networkDataToJson(downstream_network_nodes2,
...
#' Reload upstream analysis data and saved results.
#'
#' @param rv Variables list in the Shiny app.
#' @param progress Progress bar for display.
#'
...
loadRawAnalyticsData <- function(rv, progress) {
  ...
  if (file.exists(paste0(rv$outputDir, "/rna_df.RData"))) {
    ...
    rv$rna_data_mean <- rowMeans(data)
  ...
  if (file.exists(paste0(rv$outputDir, "/umap_result.RData"))) {
    load(paste0(rv$outputDir, "/umap_result.RData"))
  ...
  progress$set(0.48, detail = "Reload clustering data")
  #main cluster result reload
  if (file.exists(paste0(rv$outputDir, "/clusters.RData"))) {
    load(paste0(rv$outputDir, "/clusters.RData"))
  ...
  if (file.exists(paste0(rv$outputDir, "/gene_expression_vln_plot.RData"))) {
    load(paste0(rv$outputDir, "/gene_expression_vln_plot.RData"))
    rv$gene_expression_vln_plot <- gene_expression_vln_plot
  }
  
  if (file.exists(paste0(rv$outputDir, "/gene_expression_scatter_plot.RData"))) {
    load(paste0(rv$outputDir, "/gene_expression_scatter_plot.RData"))
  ...
  if (file.exists(paste0(rv$outputDir, "/cell_annotation.RData")) &
      file.exists(paste0(rv$outputDir, "/annotation_result.RData"))) {
    load(paste0(rv$outputDir, "/cell_annotation.RData"))
    load(paste0(rv$outputDir, "/annotation_result.RData"))
