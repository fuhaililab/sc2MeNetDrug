...
processDrugFile<-function(dataDir,drugBankInformation,progress){
  ...
  source(utilsGctx)
  source(dataGctx)
  gctx_file<-paste(dataDir,'GSE92742_Broad_LINCS_Level5_COMPZ.MODZ_n473647x12328.gctx',sep="/")
  sig_info_file <- paste(dataDir, 'GSE92742_Broad_LINCS_sig_info.txt', sep='/')
  ...
  pert_dose_unit <- as.character(sig_info[,7]) #  %     µM    -666  µL    ng/mL ng    ng/µL
  
  cell_IDs <- c("PC3", "VCAP", "A375", "A549", "HA1E", "HCC515", "HT29", "MCF7", "HEPG2")
  cidT <- which((pert_itime0 == "24") & (pert_idose0 == 10) & (pert_dose_unit == 'µM') & cell_id0 %in% cell_IDs)
  cidT <- intersect(cidT, grep("BRD-", pert_id)) # compounds
  rm(sig_info_file,sig_info,pert_id,cell_id0,pert_idose0,pert_itime0,pert_dose_unit,cell_IDs,ioGctx,utilsGctx,dataGctx)
  gc()
  ...
  f_gene <- paste(dataDir, 'GSE92742_Broad_LINCS_gene_info.txt', sep='/')
  ...
  drug_name_split<-sapply(pert_iname,function(x){strsplit(x,":")})
  ...
  rm(pert_iname,drug_name_split,drug_information_name,drug_index)
  gc()
  
  progress$set(0.8,detail="Process drug data")

  f_drug<-paste(dataDir,"GSE92742_Broad_LINCS_pert_info.txt",sep="/")
  drug_information<-read.delim(f_drug)
  drug_mapping<-merge(rankname_table,drug_information,all.x = TRUE,by="pert_id")
  rm(drug_information)
  gc()
  ...
  drugBank_drug<-toupper(as.character(drugBankInformation$drug_name))
  drug_filter<-function(x,drugBank_drug){
    x<-toupper(x)
    return(grepl("^BRD|TRCN|CMAP",x)||x%in%drugBank_drug)
    
  }
  keep_drug<-sapply(pert_iname,drug_filter,drugBank_drug=drugBank_drug)
